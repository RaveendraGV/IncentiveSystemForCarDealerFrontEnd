...
class UpdateBooking extends React.Component{
    ...
    componentDidMount(){
        axios.get("http://localhost:8080/booking/get" + this.props.params.bookingId).then(resp=>this.state({
    ...
    render(){
        return(<form className="updateform">
            <div>
                <h4>Update Car for a booking</h4>            
            <Form>
                <Form.Group className="mb-3" controlId="formBasicId">
                    <Form.Label>Enter the CarId</Form.Label>
                    <Form.Control type="text" name="carId" id="carId" onChange={this.handleChange} value={this.state.carId} />                    
                </Form.Group>
